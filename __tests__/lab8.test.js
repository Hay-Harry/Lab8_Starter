...
describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    let sdRoot = await productItem.getProperty('shadowRoot');
    let button = await sdRoot.$('button');
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for(let i = 1; i < productItem.length; i++){ //begin with 1 because step 2 click once
      let sdRoot = await productItem[i].getProperty('shadowRoot');
      let button = await sdRoot.$('button');
      await button.click();
    }
    ...
    expect(cartCountText['_remoteObject'].value).toBe("20");
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    let cartCount = await page.$('#cart-count');
    let cartCountText = await cartCount.getProperty('innerText');
    expect(cartCountText['_remoteObject'].value).toBe("0");
  }, 10000);
